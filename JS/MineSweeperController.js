...
function newGame() {
    ...
    for (var rows = 0; rows < height; rows++) {
        ...
        for (var cols = 0; cols < width; cols++) {
            ...
            btn.setAttribute('id', rows + "," + cols); // IDDDDDD GIVER RIGHT HEREEE ****
        ...
        for (let j = 0; j < width; j++, b = !b)
            if (i > height / 2)
                if (b)
                    ...
                    document.getElementById(XYtoString(i, j)).style.background = '#4a92c2'; // blue vibrant
            else
                if (b)
                    document.getElementById(XYtoString(i, j)).style.background = '#c7b22c'; // yellow
                else
                    document.getElementById(XYtoString(i, j)).style.background = '#ffe95c'; // yellow vibrant
}
...
function createField(id) {
    ...
    while (minesNum < minMines) {
        ...
        if (!mines.has(XYtoString(i, j))) {
            setMine(i, j);
            incrementSurrounding(XYtoString(i, j)); // fill around mines
        }
    }
    step(id);
}
...
function filler(id) {
    // on click on a zero, find adjecant zeros and open them
    let adj = getAdjacentes(id);
    document.getElementById(id.pointToString()).innerHTML = '';
    if (state[id.x][id.y] == 1) return; 
    state[id.x][id.y] = 1;
